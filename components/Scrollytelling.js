...
export default ({ debug }) => {
  ...
  const letterProgress = clamp(remap(progress, 0.5, 0.8, -10, 0), -10, 0);
  const letterScale = clamp(remap(progress, 0.7, 0.8, 0, 1), 0, 1);
  const trail = useTrail(7, { x: letterProgress, scale: letterScale });
  const letters = trail.map(({ x, scale }, i) =>
    interpolate(
      [x, scale],
      (o, oscale) => `translate(${(trail.length - i) * o}, 0) scale(${oscale})`
    )
  );
  return (
    <Wrapper>
      ...
      <animated.div className="circle" style={circle2Style}>{debug && `${(circle2Progress*100).toFixed(2)}%`}</animated.div>
      <animated.div className="circle white" style={circle1Style}>{debug && `${(circle1Progress*100).toFixed(2)}%`}</animated.div>
      <animated.div className="circle" style={circleStyle}>{debug && `${(circleProgress*100).toFixed(2)}%`}</animated.div>
      {
        debug &&
        <PagePercent>Page Progress {(progress * 100).toFixed(2)}%</PagePercent>
      }
      {
        debug &&
        <LogoPercent>Logo Progress {(letterScale * 100).toFixed(2)}%</LogoPercent>
      }
    </Wrapper>
  );
};
